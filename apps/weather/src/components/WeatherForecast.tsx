    ...
    <Card style={{ minWidth: "300px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        {forecast.time.map((date: string, index: number) => (
          <div key={date} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px", borderBottom: "1px solid #eee" }}>
            <span style={{ fontWeight: "bold" }}>
              {new Date(date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
            </span>
            <div style={{ display: "flex", gap: "8px" }}>
              <Badge variant="primary">{forecast.temperature_2m_max[index]}°C</Badge>
              <Badge variant="secondary">{forecast.temperature_2m_min[index]}°C</Badge>
            </div>
          </div>
        ))}
      </div>
